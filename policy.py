...
from diffusionn.transformer_for_diffusion import TransformerForDiffusion
...
class DiffusionPolicy(nn.Module):
    def __init__(self, args_override):
        ...
        model = TransformerForDiffusion(
            ...
            n_obs_steps=self.observation_horizon,
    ...
    def __call__(self, qpos, image, actions=None, is_pad=None):
        ...
        if actions is not None: # training time
            ...
            for cam_id in range(len(self.camera_names)):
                ...
                #print(type(nets['policy']['backbones']))
            ...
            #print(noisy_actions.size(),timesteps.size(), obs_cond.size())
            #noise_pred = nets['policy']['noise_pred_net'](noisy_actions, timesteps, global_cond=obs_cond)
            '''print(self.observation_horizon,
            self.action_horizon,
            self.prediction_horizon)
            print(obs_cond.size())'''
            noise_pred = model(noisy_actions, timesteps,obs_cond)
            # L2 loss
            ...
            if self.ema is not None:
                ...
                #nets = self.ema.shadow_params
                #self.ema.copy_to(nets['policy']['backbones'])
            ...
            for k in self.noise_scheduler.timesteps:
                ...
                noise_pred = model(naction,k,obs_cond)
